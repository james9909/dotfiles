...
function clean() {
    ...
    sudo apt-get remove -y "$(dpkg --get-selections | cut -f1 | grep -P "^unity-(lens|scope)-" | grep -vP "unity-(lens|scope)-(home|applications|files)" | tr "\n" " ");"
}

function install_urxvt() {
    sudo -v
    sudo apt-get install -y libperl-dev rxvt-unicode-256color x11-server-utils
    # cd /tmp
...
function install_programming_tools() {
    ...
    sudo apt-get install -y
        openjdk-8-jdk git python-dev python3 \
        software-properties-common python-pip python3-pip \
        make gparted curl \
        unity-tweak-tool \
        mpd ncmpcpp irssi \
        wicd-curses \
        indicator-cpufreq lm-sensors \
        nodejs \
        zsh tmux ranger \
        ffmpeg id3v2 \
        openssh-server sshuttle
    ...
    if [[ ! -d $HOME/.oh-my-zsh ]]; then
        ...
        ln -s "${dotfiles}/oh-my-zsh.zsh-theme" "${HOME}/.oh-my-zsh/themes/oh-my-zsh.zsh-theme"
    fi
